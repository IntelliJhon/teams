...
export async function GET(
  ...
  try {
    ...
    // Call n8n using query-param format (/webhook/order?orderId=...&token=...)
    const targetUrl = `${n8nBase.replace(/\/+$/, "")}/order?orderId=${encodeURIComponent(orderId)}&token=${encodeURIComponent(token)}`;
